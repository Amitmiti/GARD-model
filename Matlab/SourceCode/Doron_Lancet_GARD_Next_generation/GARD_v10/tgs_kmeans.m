...
elseif knum == 1,
    ...
    % For one compotype calculate the mean of the traces done above and do
    % not normalize. - Lior Segev. only if it 'cosine' metric for distance
    % normalize the centroid.
    if strcmp(clusternorm, 'cosine')
        optcentroids=optcentroids ./ (ones(size(optcentroids,1),1) * sqrt(sum(optcentroids.^2)));
    end
    
    % Omer's commented out section
...
if nargout >= 3,
    ...
    if knum == 1,
		...
		if strcmp(clusternorm, 'sqEuclidean')
			% for some reason the function below is missing. i am not why
			% would one need a sillhouette if there is only one cluster..
            % I have commented the missing function out and replaced with
            % the same function as the one in the else part. % Lior Segev 16 june 2019
            meansilh = tgs_carpet(trace(:,silhindx),'none'); % tested by
            
            % Below is what Omer did with Aron.
